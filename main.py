"""
main.py — Friday by AVI (FINAL with fixed phone routing + YouTube fix)
"""
...
class Friday:
    ...
    def _greet(self):
        ...
        msg = f"{greeting}, AVI. Main hamesha sun rahi hoon. Bas 'Friday' keh kar command do."
        self.tts.speak(msg)
        if self.gui:
            ...
            try:
                ...
                if any(re.search(pattern, lower_text) for pattern in TRIGGER_PATTERNS):
                    ...
                    if command:
                        ...
                        if self.gui:
                            self.gui.add_message("You", user_text)
                        self.tts.speak("Ji, bolo AVI.")
                        if self.gui:
                            self.gui.add_message("Friday", "Ji, bolo AVI.")
    ...
    def _process_command(self, command: str):
        ...
        if result and result.speech:
            ...
            if result.intent:
                self._execute_action(result.intent, result.slots)
    ...
    def _execute_action(self, intent: str, slots: dict):
        ...
        # Phone commands get separate handling
        if "phone" in app_str or "on phone" in app_str or "on my phone" in app_str:
            print("[DEBUG] Routing to PHONE control")
            try:
                from phone_control import phone_dispatch
                # FIXED: Passing intent so close commands work correctly
    ...
    def _shutdown(self):
        ...
        self.tts.speak_sync("Goodbye AVI. Have a great day!")
        self.memory.save()
...
class FridayGUI:
    def __init__(self, friday):
        ...
        ctk.CTkLabel(left_frame, text="Conversation", font=ctk.CTkFont(size=20, weight="bold")).pack(pady=(10,15))

        self.chat_area = scrolledtext.ScrolledText(left_frame, wrap="word", font=("Segoe UI", 14),
        ...
        self.status_label = ctk.CTkLabel(right_frame, text="Always Listening...", font=ctk.CTkFont(size=18), text_color="#00ff88")
        self.status_label.pack(pady=5)

        self.voice_btn = ctk.CTkButton(right_frame, text="🎤 Voice Input", height=60, fg_color="#00b4ff", command=self.voice_input)
        self.voice_btn.pack(pady=15, padx=30, fill="x")

        self.input_entry = ctk.CTkEntry(right_frame, placeholder_text="Type here or say 'Friday ...'", height=50)
        self.input_entry.pack(pady=10, padx=30, fill="x")

        send_btn = ctk.CTkButton(right_frame, text="Send", height=48, command=self.send_message)
        ...
        ctk.CTkButton(bottom_frame, text="Exit", fg_color="#ff3366", command=self.shutdown).pack(side="left", padx=8, expand=True)
    ...
    def load_gif(self):
        try:
            ...
            self.frames = [ImageTk.PhotoImage(frame.convert("RGBA").resize((400, 400), Image.Resampling.LANCZOS)) 
                           for frame in ImageSequence.Iterator(img)]
            self.orb_label.configure(image=self.frames[0])
        except Exception as e:
            print(f"[GIF Error] {e}")
            self.fallback_label = ctk.CTkLabel(self.orb_label.master, text="🌟", font=ctk.CTkFont(size=160))
    ...
    def send_message(self):
        ...
        threading.Thread(target=self.friday._process_command, args=(text,), daemon=True).start()
